...
LRMultiClass <- function(X, y, Xt, yt, numIter = 50, eta = 0.1, lambda = 1, beta_init = NULL){
  ...
  pk <- exp_mat / rowSums(exp_mat) 
  pos_ind <- cbind(1:N, (y+1)) # position of indicator function to be one
  objective[1] <- -sum(log(pk[pos_ind])) + 0.5*lambda*norm(beta_init, type="F")
  ...
  # Within one iteration: perform the update, calculate updated objective function and training/testing errors in %
    ...
    # Report step
    objective[iter+1] <- -sum(log(pk[pos_ind])) + 0.5*lambda*norm(beta, type="F")
